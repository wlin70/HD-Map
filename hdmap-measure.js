...
var _ = require('underscore');
var HDMap = (typeof window !== "undefined" ? window['HDMap'] : typeof global !== "undefined" ? global['HDMap'] : null);
...
HDMap.Control.Measure = HDMap.Control.extend({
  ...
  options: {
    ...
    popupOptions: {             // standard hdmap popup options http://hdmapjs.com/reference.html#popup-options
      className: 'hdmap-measure-resultpopup',
      autoPanPadding: [10, 10]
    }
  },
  initialize: function (options) {
    HDMap.setOptions(this, options);
    this.options.units = HDMap.extend({}, units, this.options.units);
  ...
  onAdd: function (map) {
    ...
    this._layer = HDMap.layerGroup().addTo(map);
  ...
  _initLayout: function () {
    var className = this._className, container = this._container = HDMap.DomUtil.create('div', className);
    ...
    // copied from hdmap
    // https://bitbucket.org/ljagis/js-mapbootstrap/src/4ab1e9e896c08bdbc8164d4053b2f945143f4f3a/app/components/measure/hdmap-measure-control.js?at=master#cl-30
    container.setAttribute('aria-haspopup', true);
    if (!HDMap.Browser.touch) {
      HDMap.DomEvent.disableClickPropagation(container);
      HDMap.DomEvent.disableScrollPropagation(container);
    } else {
    ...
    if (!HDMap.Browser.android) {
      HDMap.DomEvent.on(container, 'mouseenter', this._expand, this);
      HDMap.DomEvent.on(container, 'mouseleave', this._collapse, this);
    }
    HDMap.DomEvent.on($toggle, 'click', HDMap.DomEvent.stop);
    if (HDMap.Browser.touch) {
      HDMap.DomEvent.on($toggle, 'click', this._expand, this);
    } else {
      HDMap.DomEvent.on($toggle, 'focus', this._expand, this);
    }
    HDMap.DomEvent.on($start, 'click', HDMap.DomEvent.stop);
    HDMap.DomEvent.on($start, 'click', this._startMeasure, this);
    HDMap.DomEvent.on($cancel, 'click', HDMap.DomEvent.stop);
    HDMap.DomEvent.on($cancel, 'click', this._finishMeasure, this);
    HDMap.DomEvent.on($finish, 'click', HDMap.DomEvent.stop);
    HDMap.DomEvent.on($finish, 'click', this._handleMeasureDoubleClick, this);
  },
  _expand: function () {
  ...
  _startMeasure: function () {
    ...
    HDMap.DomEvent.on(this._container, 'mouseenter', this._handleMapMouseOut, this);

    if (!this._measureCollector) {
      // polygon to cover all other layers and collection measure move and click events
      this._measureCollector = HDMap.polygon([[90, -180], [90, 180], [-90, 180], [-90, -180]], this._symbols.getSymbol('measureCollector')).addTo(this._layer);
    ...
    this._measureVertexes = HDMap.featureGroup().addTo(this._layer);
  ...
  _finishMeasure: function () {
    ...
    HDMap.DomEvent.off(this._container, 'mouseover', this._handleMapMouseOut, this);
  ...
  _handleMeasureMove: function (evt) {
    if (!this._measureDrag) {
      this._measureDrag = HDMap.circleMarker(evt.latlng, this._symbols.getSymbol('measureDrag')).addTo(this._layer);
    } else {
  ...
  _handleMeasureDoubleClick: function () {
    ...
    if (latlngs.length === 1) {
      resultFeature = HDMap.circleMarker(latlngs[0], this._symbols.getSymbol('resultPoint'));
      popupContent = pointPopupTemplate({
        model: calced,
        humanize: humanize
      });
    } else if (latlngs.length === 2) {
      resultFeature = HDMap.polyline(latlngs, this._symbols.getSymbol('resultLine')).addTo(this._map);
      popupContent = linePopupTemplate({
        model: _.extend({}, calced, this._getMeasurementDisplayStrings(calced)),
        humanize: humanize
      });
    } else {
      resultFeature = HDMap.polygon(latlngs, this._symbols.getSymbol('resultArea'));
    ...
    popupContainer = HDMap.DomUtil.create('div', '');
    popupContainer.innerHTML = popupContent;

    zoomLink = $('.js-zoomto', popupContainer);
    if (zoomLink) {
      HDMap.DomEvent.on(zoomLink, 'click', HDMap.DomEvent.stop);
      HDMap.DomEvent.on(zoomLink, 'click', function () {
    ...
    if (deleteLink) {
      HDMap.DomEvent.on(deleteLink, 'click', HDMap.DomEvent.stop);
      HDMap.DomEvent.on(deleteLink, 'click', function () {
  ...
  _handleMeasureClick: function (evt) {
    ...
    if (!lastClick || !latlng.equals(lastClick)) { // skip if same point as last click, happens on `dblclick`
      ...
      this._measureVertexes.eachLayer(function (layer) {
        ...
        // `layer.setStyle({ className: 'layer-measurevertex'})` doesn't work. https://github.com/hdmap/hdmap/issues/2662
  ...
  _addNewVertex: function (latlng) {
    HDMap.circleMarker(latlng, this._symbols.getSymbol('measureVertexActive')).addTo(this._measureVertexes);
  },
  _addMeasureArea: function (latlngs) {
    ...
    if (!this._measureArea) {
      this._measureArea = HDMap.polygon(latlngs, this._symbols.getSymbol('measureArea')).addTo(this._layer);
    } else {
  ...
  _addMeasureBoundary: function (latlngs) {
    ...
    if (!this._measureBoundary) {
      this._measureBoundary = HDMap.polyline(latlngs, this._symbols.getSymbol('measureBoundary')).addTo(this._layer);
    } else {
...
HDMap.Map.addInitHook(function () {
  if (this.options.measureControl) {
    this.measureControl = (new HDMap.Control.Measure()).addTo(this);
  }
});

HDMap.control.measure = function (options) {
  return new HDMap.Control.Measure(options);
};
